...
- `SENTRY_DSN` (optional) the [Sentry DSN](https://docs.sentry.io/product/sentry-basics/dsn-explainer/). If defined, error messages are sent to Sentry.
...
The actions can be `create`, `read`, `update`, `delete` and `manage`, where `manage` is also a wildcard for any action.
...
A swagger UI can be visited at `/api/` which shows all endpoints that are available.
